  ...
  const options = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          title: (context) => {
            const timeFrameText = timeFrame === "daily" ? "Date" : timeFrame === "monthly" ? "Month" : "Year"
          ...
          afterLabel: (context) => {
            if (timeFrame === "daily") {
              ...
              return `Full date: ${date.toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}`
            }
            return ""
          },
        },
            ...
            {stats.map((stat, index) => (
              ...
              >
                <div className="flex flex-row items-center justify-between space-y-0">
                  ...
                  <div className={`p-2 rounded-md ${stat.bgColor}`}> {/* Simpler background, rounded-md */}
                    <stat.icon className={`h-5 w-5 ${stat.color}`} /> {/* Icon color from stat.color */}
                  </div>
                </div>
                <div>
                  <div className="text-xl font-bold text-gray-900 leading-none pb-2"> {/* Smaller font, bold */}
                    {stat.value}
                  </div>
                </div>
              </div>
      ...
      <div className='border rounded-lg'>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 px-5 mt-5">
          <div className='border w-full rounded-lg hover:shadow-md'>
            <div className="p-4">
              <div className="text-2xl font-bold text-blue-600">{getTotalOrders().toLocaleString()}</div>
              <div className="text-sm text-gray-600">Total Orders ({timeFrame})</div>
            </div>
          </div>
          <div className='border w-full rounded-lg hover:shadow-md'>
            <div className="p-4">
              <div className="text-2xl font-bold text-green-600">{getAverageOrders().toLocaleString()}</div>
              <div className="text-sm text-gray-600">Average Orders</div>
            </div>
          </div>
          <div className='border w-full rounded-lg hover:shadow-md'>
            <div className="p-4">
              <div className="text-2xl font-bold text-purple-600">{getCurrentData().datasets[0].data.length}</div>
        ...
        <div className="h-[400px] w-full p-5 pb-16">
            <p className='font-semibold mb-5'>Orders  {timeFrame.charAt(0).toUpperCase() + timeFrame.slice(1)} Overview</p>
